...
import { n as clearRateLimit, o as isPasswordConfigured, r as createSession, s as rateLimit, t as checkPassword } from "./auth_D8MRE_gT.mjs";
import { t as $$Base } from "./Base_bfGYQTWK.mjs";
import { r as logActivity } from "./supabase_DnnZyzMy.mjs";
...
var $$Login = createComponent(async ($$result, $$props, $$slots) => {
	...
	if (Astro.request.method === "POST") {
		if (!configured) error = "ADMIN_PASSWORD isn’t set on the server. Add it in Vercel, then redeploy.";
		else {
			const ip = Astro.clientAddress || "unknown";
			const limit = rateLimit(ip);
			if (!limit.allowed) error = `Too many attempts. Try again in ${limit.retryInMinutes} minute${limit.retryInMinutes === 1 ? "" : "s"}.`;
			else {
				const form = await Astro.request.formData();
				const password = String(form.get("password") ?? "");
				if (checkPassword(password)) {
					if (await createSession(Astro.cookies)) {
						clearRateLimit(ip);
						await logActivity("login", "Signed in to the dashboard");
						return Astro.redirect(safeNext, 302);
					}
					error = "Could not start a session. Check the server configuration.";
